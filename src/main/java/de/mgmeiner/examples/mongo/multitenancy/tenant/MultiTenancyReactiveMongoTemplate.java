...
final class MultiTenancyReactiveMongoTemplate implements ReactiveMongoOperations,
        ApplicationContextAware {

    private Map<String, ReactiveMongoTemplate> delegates = new HashMap<>();

    public MultiTenancyReactiveMongoTemplate(
            List<MultiTenancyReactiveMongoDatabaseFactory> mTenancyReactMongoDBFactories,
            MongoConverter mongoConverter
                                            ) {
        for (var mTenancyReactMongoDBFactory : mTenancyReactMongoDBFactories) {

            var reactMongoTemplate =
                    new ReactiveMongoTemplate(
                            mTenancyReactMongoDBFactory,mongoConverter);

            delegates.put(mTenancyReactMongoDBFactory.getTenantId(),
                          reactMongoTemplate
                         );
        }
    }
    ...
    @Override
    public <T> Flux<T> findDistinct(Query query,String field,Class<?> entityClass,
                                    Class<T> resultClass) {
        return getDelegate().flatMapMany(
                it -> it.findDistinct(query,field,entityClass,resultClass));
    }
    ...
    public <T> Flux<T> findDistinct(Query query,String field,String collectionName,
        ...
        return getDelegate().flatMapMany(
                it -> it.findDistinct(query,field,collectionName,entityClass,resultClass));
    }

    @Override
    public <O> Flux<O> aggregate(TypedAggregation<?> aggregation,String inputCollectionName,
                                 Class<O> outputType) {
        return getDelegate().flatMapMany(
                it -> it.aggregate(aggregation,inputCollectionName,outputType));
    }
    ...
    @Override
    public <O> Flux<O> aggregate(Aggregation aggregation,String collectionName,
                                 Class<O> outputType) {
    ...
    public <T> Mono<T> findAndModify(Query query,Update update,Class<T> entityClass,
                                     String collectionName) {
        return getDelegate().flatMap(
                it -> it.findAndModify(query,update,entityClass,collectionName));
    }
    ...
    public <T> Mono<T> findAndModify(Query query,Update update,FindAndModifyOptions options,
        ...
        return getDelegate().flatMap(
                it -> it.findAndModify(query,update,options,entityClass,collectionName));
    }

    @Override
    public <S,T> Mono<T> findAndReplace(Query query,S replacement,FindAndReplaceOptions options,
                                        Class<S> entityType,String collectionName,
                                        Class<T> resultType) {
        return getDelegate().flatMap(
                it -> it.findAndReplace(query,replacement,options,entityType,collectionName,
                                        resultType
                                       ));
    }
    ...
    @Override
    public <T> Flux<T> insertAll(Mono<? extends Collection<? extends T>> batchToSave,
                                 Class<?> entityClass) {
    ...
    public <T> Flux<T> findAllAndRemove(Query query,Class<T> entityClass,String collectionName) {
        return getDelegate().flatMapMany(
                it -> it.findAllAndRemove(query,entityClass,collectionName));
    }
    ...
    @Override
    public <T> Flux<ChangeStreamEvent<T>> changeStream(String database,String collectionName,
                                                       ChangeStreamOptions options,
                                                       Class<T> targetType) {
        return getDelegate().flatMapMany(
                it -> it.changeStream(database,collectionName,options,targetType));
    }

    @Override
    public <T> Flux<T> mapReduce(Query filterQuery,Class<?> domainType,Class<T> resultType,
                                 String mapFunction,String reduceFunction,
                                 MapReduceOptions options) {
        return getDelegate().flatMapMany(
                it -> it.mapReduce(filterQuery,domainType,resultType,mapFunction,reduceFunction,
                                   options
                                  ));
    }

    @Override
    public <T> Flux<T> mapReduce(Query filterQuery,Class<?> domainType,String inputCollectionName
            ,Class<T> resultType,String mapFunction,String reduceFunction,
                                 MapReduceOptions options) {
        return getDelegate().flatMapMany(
                it -> it.mapReduce(filterQuery,domainType,inputCollectionName,resultType,
                                   mapFunction,reduceFunction,options
                                  ));
    }
    ...
    private Mono<ReactiveMongoTemplate> getDelegate() {
        return Mono.subscriberContext()
                   .map(ctx -> {
                       var tenant = ctx.getOrEmpty("tenant")
